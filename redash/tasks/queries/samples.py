...
from redash import models, schema, settings, utils
from redash.models import ColumnMetadata, TableMetadata, db
...
def update_sample(data_source_id, table_name, table_id, sample_updated_at):
    ...
    persisted_columns = ColumnMetadata.query.filter(
        ColumnMetadata.exists.is_(True), ColumnMetadata.table_id == table_id,
    ...
    if (
        ...
        and parser.parse(sample_updated_at) > DAYS_AGO
    ):
