...
class TestUnusedQueryResults(BaseTestCase):
    ...
    def test_doesnt_return_live_incremental_results(self):
        ...
        qt = "SELECT 1"
        qrs = [
            self.factory.create_query_result(query_text=qt, retrieved_at=two_weeks_ago)
            for _ in range(5)
        ]
        q = self.factory.create_query(
            query_text=qt, latest_query_data=qrs[0], schedule_resultset_size=3
        )
        for qr in qrs:
    ...
    def test_deletes_stale_resultsets(self):
        qt = "SELECT 17"
        query = self.factory.create_query(query_text=qt, schedule_resultset_size=5)
        for _ in range(10):
    ...
    def test_deletes_stale_resultsets_with_dupe_queries(self):
        ...
        for _ in range(10):
            r = self.factory.create_query_result(query_text=qt)
            self.factory.create_query_resultset(query_rel=query, result=r)
        query2 = self.factory.create_query(query_text=qt, schedule_resultset_size=3)
        for _ in range(10):
